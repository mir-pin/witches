...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateActiveTheme(themeName) {
    ...
    // update the intro title style based on the theme
    updateIntroTitle(themeName);
  }

  // curved text for Theme 2
  function updateIntroTitle(themeName) {
    const introTitle = document.querySelector('.introduction-title h1');
    if (!introTitle) return;

    // theme 2: apply curved SVG text
    if (themeName === 'theme2') {
      if (!introTitle.querySelector('.curved-svg')) {
    ...
    // other themes: restore plain text
    else {
      const stdText = introTitle.querySelector('.std-text');
      if (stdText) {
        introTitle.innerHTML = stdText.innerText;
      }
    }
  }
